...
public class GenerateMapV2
{
    ...
    private GenerateMapV2(Context context)
    {
        COORD_TEX_HEIGHT = 0;
        COORD_TEX_WIDTH = 0;
        Log.d("GenerateMapV2", "GenerateMapV2 new! _CONFIG_LUT_PREVIEW_CAPTURE_USE_DIFFERENT=true");
        long l = System.currentTimeMillis();
        if(!generateFromFile(1))
        {
            if(!generateFromFile(0))
            {
                generateFromResourceRaw(context);
                DebugStatistic.vr_pip_preview_record_lut_path = "raw_lut_loaded";
            } else
            {
                DebugStatistic.vr_pip_preview_record_lut_path = "old_lut_loaded";
            }
        } else
        {
            DebugStatistic.vr_pip_preview_record_lut_path = "preview_lut_loaded";
        }
        DebugStatistic.vr_pip_preview_record_lut_size_width = COORD_TEX_WIDTH;
        DebugStatistic.vr_pip_preview_record_lut_size_height = COORD_TEX_HEIGHT;
        Log.d("GenerateMapV2", (new StringBuilder()).append("GenerateMapV2 new time=").append(System.currentTimeMillis() - l).toString());
    }
    ...
    private boolean generateFromFile(int i)
    {
        boolean flag = false;
        String s = null;
        Log.d("GenerateMapV2", "generateFromFile start");
        long l = System.currentTimeMillis();
        ...
        if(s == null)
        {
            Log.v("GenerateMapV2", "Error find res directory!");
        } else
        {
            Log.v("GenerateMapV2", (new StringBuilder()).append("find res dir =(").append(s).toString());
            mapHandle = new int[2];
            ...
            {
                Log.v("GenerateMapV2", "Error generating mapping texture.");
                release_all_texture();
            } else
            {
                ...
                if(1 == i)
                {
                    ...
                    Log.v("GenerateMapV2", "try load preview lut!");
                } else
                if(2 == i)
                {
                    as = __RES_LUT_CAPTURE;
                    Log.v("GenerateMapV2", "try load capture lut!");
                } else
                {
                    Log.v("GenerateMapV2", "try load old lut!");
                }
                if(!generateTexture_((new StringBuilder()).append(s).append("/").append(as[0]).toString(), getAlphaHandler(), true))
                {
                    release_all_texture();
                    Log.v("GenerateMapV2", (new StringBuilder()).append("res_path:").append(s).append("/").append(as[0]).append(" failed!").toString());
                } else
                if(!generateTexture_((new StringBuilder()).append(s).append("/").append(as[1]).toString(), getLeftMapHandler(), false))
                {
                    release_all_texture();
                    Log.v("GenerateMapV2", (new StringBuilder()).append("res_path:").append(s).append("/").append(as[1]).append(" failed!").toString());
                } else
                if(!generateTexture_((new StringBuilder()).append(s).append("/").append(as[2]).toString(), getRightMapHandler(), false))
                {
                    release_all_texture();
                    Log.v("GenerateMapV2", (new StringBuilder()).append("res_path:").append(s).append("/").append(as[2]).append(" failed!").toString());
                } else
                {
                    Log.d("GenerateMapV2", (new StringBuilder()).append("generateFromFile done Time:").append(System.currentTimeMillis() - l).toString());
                    flag = true;
                }
            }
        }
...
//_L5:
        ...
        } else
        {
            ...
            {
                Log.v("GenerateMapV2", "Error find lutArray!");
            } else
            {
                ...
                {
                    release_all_texture();
                    Log.v("GenerateMapV2", (new StringBuilder()).append("res_path:").append(s).append("/").append(lutunderpathinfo.filenames[0]).append(" failed!").toString());
                } else
                ...
                {
                    release_all_texture();
                    Log.v("GenerateMapV2", (new StringBuilder()).append("res_path:").append(s).append("/").append(lutunderpathinfo.filenames[1]).append(" failed!").toString());
                } else
                ...
                {
                    release_all_texture();
                    Log.v("GenerateMapV2", (new StringBuilder()).append("res_path:").append(s).append("/").append(lutunderpathinfo.filenames[2]).append(" failed!").toString());
                } else
                {
                    Log.d("GenerateMapV2", (new StringBuilder()).append("generateFromFile done Time:").append(System.currentTimeMillis() - l).toString());
                    flag = true;
...
//        if(Utils.test_camera_res_read("/persist/lib/camera_res").result == 0)
 ...
 //         goto _L5
    ...
    {
        Log.d("GenerateMapV2", "generateFromResourceRaw start");
        mapHandle = new int[2];
        alphaHandle = new int[1];
        GLES20.glGenTextures(2, mapHandle, 0);
        GLES20.glGenTextures(1, alphaHandle, 0);
        ...
        {
            Log.d("GenerateMapV2", (new StringBuilder()).append("generateFromResourceRaw Time:").append(System.currentTimeMillis() - l).toString());
            return;
        }
    }
    ...
    {
        boolean flag1 = true;
        Log.v("GenerateMapV2", (new StringBuilder()).append("res_path:").append(s).append(" texObj:").append(i).append(" is_alpha:").append(flag).toString());
        File file = new File(s);
        if(file == null || !file.exists())
        {
            Log.v("GenerateMapV2", (new StringBuilder()).append("generateTexture_ : res_path=").append(s).append("File is not find").toString());
            flag1 = false;
        } else
        {
            try
            {
                ...
                if(flag)
                {
                    ...
                    Log.i("GenerateMapV2", (new StringBuilder()).append("------- size width*height : ").append(COORD_TEX_WIDTH).append("x").append(COORD_TEX_HEIGHT).toString());
                }
            ...
            catch(IOException ioexception)
            {
                ...
                Log.v("GenerateMapV2", (new StringBuilder()).append("generateTexture_ failed excep:").append(ioexception).toString());
                flag1 = false;
            }
        }
        return flag1;
    ...
    public static GenerateMapV2 getInstance(Context context)
    {
        ...
        if(gMap != null)
            ...
            obj = "null";
        Log.v("GenerateMapV2", stringbuilder.append(obj).toString());
        if(gMap == null)
            gMap = new GenerateMapV2(context);
        return gMap;
    }
    ...
    private boolean uploadTexture_(Context context, int i, int j, boolean flag)
    {
        ...
        COORD_TEX_HEIGHT = (0xff & abyte1[0]) << 8 | 0xff & abyte1[1];
        COORD_TEX_WIDTH = (0xff & abyte1[2]) << 8 | 0xff & abyte1[3];
        Log.i("GenerateMapV2", (new StringBuilder()).append("    LUT size width*height=").append(COORD_TEX_WIDTH).append("*").append(COORD_TEX_HEIGHT).toString());
        byte abyte0[] = new byte[4 * (COORD_TEX_WIDTH * COORD_TEX_HEIGHT)];
...
_L1:*/
    ...
    static final String TAG = "GenerateMapV2";
    ...
    private static GenerateMapV2 gMap = null;
